...
abstract class IntPipeline<E_IN>
    ...
    public final int reduce(int identity, IntBinaryOperator op) {
        if ((isParallel() == false) || (SumatraUtils.getOffload() == false)) {
            return evaluate(ReduceOps.makeInt(identity, op));
        } else {
            String reducerName = PipelineInfo.getIntReduceTargetName(op);
            if (reducerName != null ) {
                TerminalOp<Integer, Integer> terminalOp = ReduceOps.makeInt(identity, op);
                assert getOutputShape() == terminalOp.inputShape();
                if (getLinkedOrConsumed()) {
                    throw new IllegalStateException("stream has already been operated upon");
                }
                setLinkedOrConsumed();
                ReduceOps.ReduceOp reduceOp = (ReduceOps.ReduceOp) terminalOp;
                return reduceOp.evaluateOffloadableIntReduce(this, sourceSpliterator(terminalOp.getOpFlags()), reducerName, identity);
            } else {
                assert false : "Unknown reduce offload:" + reducerName;
                return evaluate(ReduceOps.makeInt(identity, op));
            }
        }
    }
    ...
    public final OptionalInt reduce(IntBinaryOperator op) {
        if ((isParallel() == false) || (SumatraUtils.getOffload() == false)) {
            ...
            if (reducerName != null ) {
                ...
                setLinkedOrConsumed();
                ReduceOps.ReduceOp reduceOp = (ReduceOps.ReduceOp) terminalOp;
                return OptionalInt.of(reduceOp.evaluateOffloadableIntReduce(this, sourceSpliterator(terminalOp.getOpFlags()), reducerName, identity));
            } else {
                assert false : "Unknown reduce offload:" + reducerName;
                return evaluate(ReduceOps.makeInt(op));
            }
